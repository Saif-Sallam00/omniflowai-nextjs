...
- [ ] T001 Run the existing quality gate as a baseline (`npm run check`, `npm run lint`, `npm run build`) and confirm slice 1A's quickstart checks still pass — establishes a clean starting point so any later failure is attributable to this slice's own changes, not pre-existing drift.
