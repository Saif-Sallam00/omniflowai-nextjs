...
| Chrome renders statically, no per-request API (Edge Case / SC-006) | Inspect `components/site-shell.tsx`, `app/(en)/(public)/layout.tsx`, `app/ar/(public)/layout.tsx` | No `headers()`/`cookies()` calls anywhere in the chrome or layouts |
| Admin does not inherit public chrome | Load `/admin/auth` and inspect the rendered page | No public header, nav, footer, or language switcher present — `app/(en)/layout.tsx` (admin's root layout) does not import `SiteShell` |
...
| Reusability — zero per-page reimplementation (unchanged from 1A) | Inspect `app/(en)/(public)/solutions/page.tsx` and `app/ar/(public)/solutions/page.tsx` | Each calls `buildPageMetadata` with only its own `path`/`title`/`description` — no inline canonical/OG/hreflang construction |
